...
RegisterNetEvent('apartments:server:CreateApartment', function(type, label)
    ...
    TriggerClientEvent('QBCore:Notify', src, "Apart".."("..label..")")
...
RegisterNetEvent('apartments:server:UpdateApartment', function(type, label)
    ...
    TriggerClientEvent('QBCore:Notify', src, "Berhasil Mengganti Apart")
    TriggerClientEvent("apartments:client:SetHomeBlip", src, type)
end)
...
RegisterNetEvent('server:test:buyapart', function(type, label)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local num = CreateApartmentId(type)
    local apartmentId = tostring(type .. num)
    ...
    Player.Functions.RemoveMoney("bank", Apartments.Hargabeli)
    TriggerClientEvent('QBCore:Notify', src, "Berhasil Menyewa Apart"..label)
end)

RegisterNetEvent('server:test:sellapart', function(args)
  --  print(args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    MySQL.Async.execute('DELETE FROM apartments WHERE type = ? AND citizenid = ?',
        {args, Player.PlayerData.citizenid})
    TriggerClientEvent('QBCore:Notify', src, "Berhenti Sewa Apartment")
   --- print(Player.PlayerData.citizenid)
end)

RegisterCommand("testseminggu", function(source)
    bayarwoi()
  --  TriggerClientEvent('QBCore:Notify', source, 'Memabayar Sewa apartment')
end,'admin')

function PemilikApartmet()
    local hasil = MySQL.Sync.fetchAll('SELECT citizenid FROM apartments', {})
	if hasil[1] ~= nil then
        local citizenid = nil
        for k = 1, #hasil, 1 do
            ket = hasil[k]
            --print(ket)
            citizenid = ket.citizenid
           -- print(ket)
            local jumlah = Apartments.Hargasewa
            local inisiapa = QBCore.Functions.GetPlayerByCitizenId(citizenid)
     --       print(inisiapa)
            inisiapa.Functions.RemoveMoney('bank',jumlah)
        end
    end
end

function bayarwoi(d, h, m) 
    if d == 1 then
      --  print(d,h,m)
    PemilikApartmet()
    end
end
TriggerEvent('cron:runAt', 9, 59, bayarwoi)